...
def hexagons_dataframe_to_geojson(df_hex, hex_id, geometry, value, file_output = None):
    """
...
def deep_learning_model(data: pd.DataFrame, known_latlon: pd.DataFrame, unknown_latlon: pd.DataFrame, parameters: Dict, pollutant_name):
    ...
    Returns:
        None
        Exports the Interpolated Data (numpy.array) to csv file {pollutant_name}_interpolated_data.csv with rows being unknown locations and columns being time series data
    """
    ...
    spatio_temporal_observations, data_scaler, time_series_data = pollutant_data_preprocessing(data, parameters["past_data_range"])
    ...
    # Concatenate Interpolated Data and Observed Data
    Whole_Data = np.concatenate((Interpolated_Data, time_series_data), axis = 0)

    # Create DataFrame for Binned_Data
    Binned_Data = pd.DataFrame(Whole_Data)

    # Assign Latitude and Longitude data to Binned_Data DataFrame
    Binned_Data[['latitude','longitude']] = pd.concat([unknown_latlon, known_latlon]).reset_index().drop('index', axis = 1)

    # Create new column for hex_id and get hex ids for all locations
    Binned_Data['hex_id'] = Binned_Data.apply(lambda row : geoToH3(row['latitude'],row['longitude']), axis = 1)

    #Viz_Data = pd.DataFrame(columns = ['data','hex_id'])
    #Viz_Data['data'] = Interpolated_Data[:,-1]
    #Viz_Data['hex_id'] = unknown_latlon.apply(lambda row : geoToH3(row['latitude'],row['longitude']), axis = 1)
    #Hex_Group = Viz_Data.groupby("hex_id", as_index=False).agg({"data": "sum"})

    #geojson_data = hexagons_dataframe_to_geojson(df_hex = Hex_Group, hex_id = 'hex_id', value = 'data', geometry = 'geometry')
    #print(geojson_data)
    #hexmap = choropleth_map(df_aggreg = Hex_Group, color_map_name = "Blues", column_name = "value")
    #hexmap.show()
    #fig, ax = plt.subplots(figsize=(16,12), dpi=300)
    #hb = ax.hexbin(Binned_Data['latitude'].to_numpy(), Binned_Data['longitude'].to_numpy(), C=Binned_Data.iloc[:,-4], gridsize=parameters["grid_length"], cmap='gist_rainbow') #, cmap='gist_rainbow_r'
