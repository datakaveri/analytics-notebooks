...
def get_imputated_pollutant_data(sensor_ids: pd.DataFrame, time_synchronized_data, parameters: Dict):
    ...
    for column in time_synchronized_data.columns[5:]:
        ...
        pollutant_imputated_data = pollutant_data.drop_duplicates(subset='observationDateTime', keep="first").reset_index().drop(['index'],axis=1)

        # Convert observationDateTime column to datetime object.
        pollutant_imputated_data.observationDateTime = pd.to_datetime(pollutant_imputated_data.observationDateTime, format='%Y-%m-%d %H:%M:%S%Z')

        # Save updated pollutant imputated data data
        pollutant_imputated_data.to_parquet('data/05_model_input/'+col[0]+'_imputated_data.parquet')

        # Create an empty DataFrame
        pollutant_forecast_data = pd.DataFrame(columns = pollutant_imputated_data.columns)

        # For loop for each imputated sensor data
        for column in pollutant_forecast_data.columns[1:]:

            # If the current sensor is the 0th sensor or column name is sensor_0
            if column == "sensor_0":

                # Get single sensor data
                sensor_data = pollutant_imputated_data[["observationDateTime", column]]

                # Get forecasted sensor data from imputated data
                forecast_data = get_forecasted_pollutant_data(sensor_data, parameters["forecast_period"])

                # Store observationDateTime from forecast
                pollutant_forecast_data["observationDateTime"] = forecast_data["ds"]

                # Store forecasted single sensor data
                pollutant_forecast_data[column] = forecast_data["yhat"]
        ...
        print(pollutant_forecast_data)
        # Save updated pollutant forecasted data
        pollutant_forecast_data.to_parquet('data/05_model_input/'+col[0]+'_imputated_forecasted_data.parquet')


def get_forecasted_pollutant_data(pollutant_imputated_data: pd.DataFrame, forecast_period):
    """
    Perform forecast on imputated data on the given forecast period and append it to the past data.

    Args:
        pollutant_imputated_data (pandas.core.frame.DataFrame): imputated data after preprocessing, and time synchronization - Output of get_imputated_pollutant_data function.
        forecast_period: Required forecast data defined in parameters/data_processing.yml.
    ...
    # Assign Date Time data to 'ds' column in column_data
    sensor_data['ds'] = pollutant_imputated_data['observationDateTime'].values

    # Assign imputated_data to column data
    sensor_data['y'] = pollutant_imputated_data.iloc[:,-1].values
    ...
    # Create future DataFrame with 24 hours forecast
    future = m.make_future_dataframe(periods=forecast_period*96, freq='15min') #, include_history=False)
